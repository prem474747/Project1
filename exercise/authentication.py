...
def adminhome():
    ...
    pending = Button(adminpage, test="pending List", command="pendinglist", font=(
...
def pendinglist():
    ...
    pendingframe.grid()
    cursor.execute('select * from details where verify =False')
    a = cursor.fetchall()
...
def approvedlist():
    ...
    pendingframe.grid()
    cursor.execute('select * from details where verify =True')
    a = cursor.fetchall()
    Label(pendingframe, text='ID', font=('Arial', 13)).grid(row=4, column=0)
    Label(pendingframe, text='Name', font=('Arial', 13)).grid(row=4, column=1)
    Label(pendingframe, text='Mail', font=('Arial', 13)).grid(row=4, column=2)
    ...
    for i in range(rows):
        for j in range(cols):
            s = Entry(pendingframe, font=('calibri', 11))
            s.grid(row=i+5, column=j)
            s.insert(END, a[i][j][0])


def approve(id):
    cursor.execute('update details set verify = True where id=%s', [id])
    db.commit()
